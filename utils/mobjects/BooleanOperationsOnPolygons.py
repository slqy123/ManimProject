...
def set_split(mobs, case, color_list):
    return_group = VGroup()
    for i in case:
        inter_list = []
        pop_list = []
        for j in range(len(i)):
            if i[j] == 1:
                inter_list.append(mobs[j])
            else:
                pop_list.append(mobs[j])
        mob = inter_list.pop()
        while inter_list:
            mob = PolygonIntersection(mob, inter_list.pop())
        while pop_list:
            mob = PolygonSubtraction(mob, pop_list.pop())
        mob.set_fill(color=color_list.pop(0), opacity=0.7).set_stroke(color=WHITE, width=2, opacity=1)
        return_group.add(mob)
    return return_group
